...
void btc_a2dp_source_setup_codec(void)
{
    ...
    // Patch: Thanks, Espressif! Now I have to copy the entire bt component to
    // my source tree to change 3 lines of hardcoded value - GrieferPig
...
static void btc_a2dp_source_enc_init(BT_HDR *p_msg)
{
    ...
    a2dp_source_local_param.btc_aa_src_cb.TxAaMtuSize = ((BTC_MEDIA_AA_BUF_SIZE - BTC_MEDIA_AA_SBC_OFFSET - sizeof(BT_HDR)) < pInitAudio->MtuSize) ? (BTC_MEDIA_AA_BUF_SIZE - BTC_MEDIA_AA_SBC_OFFSET - sizeof(BT_HDR)) : pInitAudio->MtuSize;
...
static void btc_a2dp_source_enc_update(BT_HDR *p_msg)
{
    ...
    // if (a2dp_source_local_param.btc_aa_src_cb.is_tx_timer)
    {
        a2dp_source_local_param.btc_aa_src_cb.TxAaMtuSize = ((BTC_MEDIA_AA_BUF_SIZE -
                                                              BTC_MEDIA_AA_SBC_OFFSET - sizeof(BT_HDR)) < pUpdateAudio->MinMtuSize)
                                                                ? (BTC_MEDIA_AA_BUF_SIZE - BTC_MEDIA_AA_SBC_OFFSET - sizeof(BT_HDR))
                                                                : pUpdateAudio->MinMtuSize;
        ...
        do
        {
            if (pstrEncParams->s16NumOfBlocks == 0 || pstrEncParams->s16NumOfSubBands == 0 || pstrEncParams->s16NumOfChannels == 0)
            {
...
static UINT8 btc_get_num_aa_frame(void)
{
    ...
    case BTC_MEDIA_TRSCD_PCM_2_SBC:
    {
        ...
        if (a2dp_source_local_param.last_frame_us != 0)
        {
            ...
            us_this_tick = (now_us > a2dp_source_local_param.last_frame_us) ? (now_us - a2dp_source_local_param.last_frame_us) : (now_us + 86400000000ull - a2dp_source_local_param.last_frame_us);
#endif
        }
...
BOOLEAN btc_media_aa_read_feeding(void)
{
    ...
    UINT16 bytes_needed = blocm_x_subband * a2dp_source_local_param.btc_aa_src_cb.encoder.s16NumOfChannels *
                          a2dp_source_local_param.btc_aa_src_cb.media_feeding.cfg.pcm.bit_per_sample / 8;
    static UINT16 up_sampled_buffer[SBC_MAX_NUM_FRAME * SBC_MAX_NUM_OF_BLOCKS * SBC_MAX_NUM_OF_CHANNELS * SBC_MAX_NUM_OF_SUBBANDS * 2];
    static UINT16 read_buffer[SBC_MAX_NUM_FRAME * SBC_MAX_NUM_OF_BLOCKS * SBC_MAX_NUM_OF_CHANNELS * SBC_MAX_NUM_OF_SUBBANDS];
...
static void btc_media_aa_prep_sbc_2_send(UINT8 nb_frame)
{
    ...
    while (nb_frame)
    {
        ...
        do
        {
            ...
            memset(a2dp_source_local_param.btc_aa_src_cb.encoder.as16PcmBuffer, 0, blocm_x_subband * a2dp_source_local_param.btc_aa_src_cb.encoder.s16NumOfChannels);
        ...
        } while (((p_buf->len + a2dp_source_local_param.btc_aa_src_cb.encoder.u16PacketLength) < a2dp_source_local_param.btc_aa_src_cb.TxAaMtuSize) && (p_buf->layer_specific < 0x0F) && nb_frame);
